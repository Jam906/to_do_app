...
class GdThumb extends ThumbBase
{
	...
	public function resize1 ($maxWidth = 0, $maxHeight = 0)
	{
		...
		$this->calcImageSize1($maxWidth, $maxHeight);
	...
	protected function calcWidth1($width, $height)
	{
	...
	protected function calcHeight1($width, $height)
	{
		$newHeightPercentage	= (100 * $this->maxHeight) / $height;
		$newWidth 				= ($width * $newHeightPercentage) / 100;
	...
	protected function calcImageSize1($width, $height)
	{
		$newSize = array
		(
			'newWidth'	=> $width,
		...
		if ($this->maxWidth > 0)
		{
			$newSize = $this->calcWidth1($width, $height);
			
			if ($this->maxHeight > 0 && $newSize['newHeight'] > $this->maxHeight)
			{
				//$newSize = $this->calcHeight($newSize['newWidth'], $newSize['newHeight']);
				$newSize = $this->calcHeight1($width, $height);
			}
		}
